...
****************************
	...
	*     do Stata/05_underbidding_matching_robustness.do
...
// 	*****************************************************************
	...
	******************* Basic Regressions *****************
	...
	esttab reg1 reg2 reg3 reg4 reg5 using "underbidding_w_autoregressive_variations_no_uri.csv", replace r2 aic bic scalar(F)	
	...
	** save and output match results
	...
	matrix teresults_xpose = teresults'
	matrix list teresults_xpose
	esttab matrix(teresults_xpose) using "active_match_results_no_uri.tex", replace ///
	cells("b(fmt(%5.3f)) se(fmt(%5.3f)) z(fmt(%5.3f)) pvalue(fmt(%5.3f))")
	
	esttab matrix(teresults_xpose) using "active_match_results_no_uri.csv", replace ///
	cells("b(fmt(%5.3f)) se(fmt(%5.3f)) z(fmt(%5.3f)) pvalue(fmt(%5.3f))")
	...
	** save and output match balance summary
	...
	matlist match_size
	esttab matrix(match_size) using "active_match_size_no_uri.tex", replace
	esttab matrix(match_size) using "active_match_size_no_uri.csv", replace


	matlist match_balance
	esttab matrix(match_balance) using "active_match_balance_no_uri.tex", replace
...
// 	drop matches*
	...
	** save and output match results
	...
	matrix teresults_xpose = teresults'
	matrix list teresults_xpose
	esttab matrix(teresults_xpose) using "active_match_results_price_cap_gap.tex", replace ///
	cells("b(fmt(%5.3f)) se(fmt(%5.3f)) z(fmt(%5.3f)) pvalue(fmt(%5.3f))")
	
	esttab matrix(teresults_xpose) using "active_match_results_price_cap_gap.csv", replace ///
	cells("b(fmt(%5.3f)) se(fmt(%5.3f)) z(fmt(%5.3f)) pvalue(fmt(%5.3f))")
	...
	** save and output match balance summary
	...
	matlist match_size
	esttab matrix(match_size) using "active_match_size_price_cap_gap.tex", replace
	esttab matrix(match_size) using "active_match_size_price_cap_gap.csv", replace


	matlist match_balance
	esttab matrix(match_balance) using "active_match_balance_price_cap_gap.tex", replace
...
// 	drop matches*
	...
	** save and output match results
	...
	matrix teresults_xpose = teresults'
	matrix list teresults_xpose
	esttab matrix(teresults_xpose) using "active_match_results_ar10_pcgap.tex", replace ///
	cells("b(fmt(%5.3f)) se(fmt(%5.3f)) z(fmt(%5.3f)) pvalue(fmt(%5.3f))")
	
	esttab matrix(teresults_xpose) using "active_match_results_ar10_pcgap.csv", replace ///
	cells("b(fmt(%5.3f)) se(fmt(%5.3f)) z(fmt(%5.3f)) pvalue(fmt(%5.3f))")
	...
	** save and output match balance summary
	...
	matlist match_size
	esttab matrix(match_size) using "active_match_size_ar10_pcgap.tex", replace
	esttab matrix(match_size) using "active_match_size_ar10_pcgap.csv", replace


	matlist match_balance
	esttab matrix(match_balance) using "active_match_balance_ar10_pcgap.tex", replace
